...
***********************************************************************************************
	...
	logical controlla_attivazione !BERNARD
      ...
      real*8 segno, str(8), pes, sumpes, pesk
	real*8 sumpesk, h_totale, V_totale,ValoreEsterno
      
	
	real*8 dh_cost(8), h_affluito, h_defluito, DHH(8), DHH_tot
	real*8 peso_e(8), alfa(40), DDH(8), ddh_tot, peso_s(8)

	real*8 control2, control3

      real*8 DT
      real*8 zero, coeff, cel, cel_max
	real*8  h_vol, h_vol_str, Qtot, beta, betamin
	real*8 h_fin_destinaz_max, h_fin_origine, h_fin(8), vero, dh_cost_tot
	real*8 h_iniz_destinaz_max, allarme2,  hh, tempo_iniziale
	real*8 DT_medio, t_simulaz_finale, t_sugg
      REAL*8 h_fine
	real*8 r_ic, r_ir, volume_entrato(100)

**************************************************** MARZO 09 Boundary condition
	real*8 mezzo_riga, mezzo_colonna, lato_bc(8), V_total, V_eros
	real*8 V_fuori_uscito_totale, colonne_dispari, righe_dispari
	real*8 V_entrato, V_netto, V_eroso, V_depositato, urca 
	real*8 dh_cost_max, diff_max, h_origine, diff_elevaz, h_finn
	real*8 DHH_max, V_dep_step, V_eros_step, h_sorgente, h_striscia
****************************************************


      real*8 h_vol_str25, ddh_tot25, h_vol_str28, ddh_tot28, dh25, dh28
	real*8 dh25fing, dh28fing, dh25_26g, dh28_27g, dh25_26s, dh28_27s
	real*8 dh25fins, dh28fins
	real*8 h_solido_pre, pippo1, con_prima
	
	real*8 ttt1, ttt2, ttt3, ttt4, ttt5, tt6, tt7
      real*8 maxSezInt
      
      real*8 Vaffluito, Vaffluito_striscia1_tot, V_intrappolato
      real*8 V_celle_tot, V_solido_eroso, V_solido_eroso_step
      real*8 V_solido_input_DT_1, V_solido_depositato_step
      real*8 V_solido_depositato, V_solido, V_celle, V_entrato_dt_1
      real*8  V_affluito, Volume_intrappolato
      real*8 V_solid_input_DT_1,VOLUME_ENTRATO_IDRO,VOLSOL_ENTRATO_IDRO
      real*8 VOLUME_ENTRATO_IDRO_1,VOLSOL_ENTRATO_IDRO_1
      real*8 flag_int_tempo_finale 
	...
	i_qhh_row = (/  0,  1, 0, -1/) 
      ...
      DFRM_VERSION ='      DFRM 2 AdB set2022     doppia precisione'
      DFRM_VERSION1 ="   Linux verisone 29/10/2024 DP     "
      
      !call getarg (1,buffer) !Legge *.BAT  BERNARD
      !
      !fileComandi=trim(adjustl(buffer))
      
     !!  fileComandi='F:\rounding_error_test_DFRM\Simulazioni\Cancia2009\
     !!11_CPU\5SIM\5SIM_ADB_round_dt_e_v.clm'
...
!	endif
	...
	! endif
      ...
      Write (0,'('' Nome simulazione= '',a40)')fileEle(1:(retint1-1))
      Write (10,'('' Nome simulazione = '',a40)')fileEle(1:(retint1-1))
      write (0,*)
 ...
 !   	  allocate (ic_str1(max_celstr,N_stati))
  !      allocate (ir_str1(max_celstr,N_stati))
	  ...
	  allocate (ele(0:no_columns+1,0:no_rows+1)) !CORRETTO BERNARD 16/07/2021
        allocate (ele1(0:no_columns+1,0:no_rows+1))
...
****************************************************************************************
	...
	write(10,*)
      ...
      do ir = 1,no_rows
       ...
       read (11) (ele1(ic,ir),ic=1,no_columns) ! raster elevazioni
...
!	      fileErodibilita = fileErodibilita(1:retint-1)//'_control_erosion'
	...
	if (control1.eq.1.0) then
      allocate(Suolo1(no_columns,no_rows))
      
      do ir = 1,no_rows
          read(69) (Suolo1(ic,ir),ic=1,no_columns)
      end do
      Suolo = Suolo1
...
!$OMP& schedule(monotonic: dynamic)
	...
	1,ir_routing(ij))*sqrt(9.81*h(ic_routing(ij),ir_routing(ij))*
     ...
     1 + sqrt(9.81*h(ic_routing(ij),ir_routing(ij)))
          ...
          if (cel.gt.cel_max) then
              cel_max = cel
              ic_cel_max = ic_routing(ij)
              ir_cel_max = ir_routing(ij)
          endif    
       endif
...
c Point 3.3 Calcolo dt
      ...
      if (cel_max.gt.0.0) then
       ...
       IF (cel_max.gt.30.0 ) cel_max = 50.0  !  8 Settembre 2017
	...
	write(10,'(" time step e tempo di simulazione (secondi)",1x,
     1f12.5,2x,5f25.5)') DT, t_dopo,V_entrato, V_solid_input, 
     1 V_depositato, V_eroso
      
      V_eroso = 0.0
	V_depositato = 0.0
      
      if (t_dopo .gt. 226.8)then
          continue
      endif    
...
**************************************************************************
	...
	if (j_vel_max.ne.0)then
         dh_eros_tot = dh_eros_tot + dh_eros(j_vel_max)
      endif
...
*************************************************************************************************
	...
	allocate (erosione_finale2(no_columns,no_rows)) 
	 ...
	 do jj = 1, no_columns
      ...
      h_finale2(jj,j) =           real(esterno)
	erosione_finale2(jj,j) =    real(esterno)
	ele_finale2(jj,j) =         real(esterno)
	h_tot_finale2(jj,j) =       real(esterno)  
	conc_finale2(jj,j) =        real(esterno)   
	

	Area_dep_eros_finale2(jj,j) = real(esterno)
	 ...
	 enddo
      ...
      h_finale2(jj,j) = real(h(jj,j))
	erosione_finale2(jj,j) =  real(Eros_tot(jj,j))
	ele_finale2(jj,j) = real(ele(jj,j) )
	h_tot_finale2(jj,j) = real(h_tot(jj,j)   )
	conc_finale2(jj,j) = real(conc(jj,j)  )
...
**********  DEPOSITO h < h_routing  **************   21/12/2010
	 ...
	 do jj = 1, no_columns
      ...
      h_finale(jj,j) = real(esterno)
	erosione_finale(jj,j) = real(esterno)
	ele_finale(jj,j) = real(esterno)
	solid_tot_finale(jj,j) = real(esterno)
	!Erosion_fin(jj,j) = esterno
	h_tot_finale(jj,j) = real(esterno) ! 30/1/2015
	conc_finale(jj,j) = real(esterno)  ! 30/1/2015
	

	Area_dep_eros_finale(jj,j) = real(esterno)
	...
	h_finale(jj,j) = 0.0
	erosione_finale(jj,j) = real(Eros_tot(jj,j) + h(jj,j))
	ele_finale(jj,j) = real(ele(jj,j) + h(jj,j))
	Solid_tot_finale(jj,j) = real(C_fondo(jj,j)*
     1 Eros_tot(jj,j)+Conc(jj,j)*h(jj,j))
	!Erosion_fin(jj,j) = 0.0
	conc_finale(jj,j) = 0.0  ! 30/1/2015
      ...
      h_finale(jj,j) = real(h(jj,j))
	erosione_finale(jj,j) =  real(Eros_tot(jj,j))
	ele_finale(jj,j) = real(ele(jj,j) )
	solid_tot_finale(jj,j) = real(C_fondo(jj,j)*Eros_tot(jj,j))
	!Erosion_fin(jj,j) = Eros_tot(jj,j)
	h_tot_finale(jj,j) = real(h_tot(jj,j))    ! 30/1/2015
	conc_finale(jj,j) = real(conc(jj,j))  ! 30/1/2015
	...
	total_solid = total_solid + solid_tot_finale(ic,ir)
	...
	write(10,'("dopo il deposito delle proofondita minori di quella minima 
     1per la propagazione")')
      write(30,'("dopo il deposito delle proofondita minori di quella
     1 minima per la propagazione")')
	
	write(210000,'("dopo il deposito delle proofondita minori di quella 
     1 minima per la propagazione")')
...
* **************************************************************
      ...
      real*8,intent(in) :: h0,hj
	real*8 sen_tetaj, peso_peso, slopej, tetaj, dx
...
* **************************************************************
      ...
      integer j, i_sh_col(8), i_sh_row(8)
      real*8,intent(IN) :: h0,hj
	real*8 sen_tetaj, peso_peso, slopej, tetaj, dx, dh
...
* **************************************************************
	...
	integer ic_0, ir_0, icj_0, irj_0, j, icont
      real*8 sen_tetaj_s, sen_tetatot_s
...
c      per celle sorgenti
	...
	real*8 sen_tetatot3,sen_tetaj_sss
	...
	if (val_sorg(icj_1,irj_1).ne.100) then
           ...
           call kern (ele(ic_1,ir_1),ele(icj_1,irj_1),
     1       jjj,sen_tetaj_sss,lato_cella)
...
c      per celle striscia
	...
	real*8 sen_tetatot2, sen_tetaj_ss
...
****************************************************************************
	...
	subroutine deposito_inferiore(DT,j,iii,dh_e_s,U_s)
      ...
      implicit none
      integer  j,  iii
	real*8 DT, er, dh_neg_s, dh_e_s, U_s
...
****************************************************************************
      ...
      implicit none
      integer  j, iii
	real*8 DT, ang_new, er, dh_neg_s, dh_e_s, U_stra_s
...
! ****************************************************************************
	...
	subroutine deposito_Belangier_inferiore(DT,j,iii,ang_new,
     1 dh_e_s,U_stra_s)
      ...
      implicit none
      integer  j, iii
	real*8 DT, er, dh_neg_s, dh_e_s, U_stra_s, ang_new

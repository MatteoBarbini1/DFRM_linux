...
c Point 3.3 Calcolo dt
      ...
      if (cel_max.gt.0.0) then
       ...
       IF (cel_max.gt.30.0 ) cel_max = 50.0  !  8 Settembre 2017
        ...
        DT = round(Courant*lato_cella/cel_max,round_term)
        !DT =0.5 !CEL_fissa
...
c point 4.3.1  calcolo portata defluente nella j-esima cella gravità     
	...
	ir1 = ir_s(ic,ir,j)
      ...
      !  3/12/2017
      UU(j) = round(coeff*peso(ic,ir,j)*sqrt(9.81*
     1 h(ic,ir)*senteta(ic,ir,j))*Ch(ic,ir),round_term)
...
c      calcolo portata defluente nella j-esima cella
	...
	UU(j) = 0.0
      ...
      if (h_tot(ic,ir).gt.h_tot(ic1,ir1)) then
        ...
        UU(j) = round(coeff*peso(ic,ir,j)*sqrt(9.81*h(ic,ir)*senteta(ic,
     1ir,j))*Ch(ic,ir),round_term)
...
c point 4.3.3   controllo oscillazioni     	
	...
	enddo
      ...
      coeff = round(coeff*(1-0.5*(h_fin_destinaz_max-h_fin_origine)
     1 /h_vol_str),round_term)
...
c      calcolo portata defluente nella j-esima cella
	...
	UU(j) = 0.0
      ...
      if (h_tot(ic,ir).gt.h_tot(ic1,ir1)) then
        ...
        UU(j) =  round(coeff*peso(ic,ir,j)*sqrt(
     19.81*h(ic,ir)*senteta(ic,ir,j))*Ch(ic,ir),round_term)
...
c      calcolo della profondita' nella cella j-esima
      dh_cost(j) = round(Q(j)*DT/(lato_cella*lato_cella),round_term)
...
c point 4.3.5.1 calcolo erosione
	...
	call erosione(DT,j,iii,dh_eros(j),UU(j))
      ...
      dh_neg =  round(dh_eros(j),round_term)
...
**************************************************************************
	...
	if (j_vel_max.ne.0)then
         dh_eros_tot = dh_eros_tot +round(dh_eros(j_vel_max),round_term)
      endif
...
c      ALGORITMO DI DEFLUSSO
	...
	if (DHH(j).gt.zero) then
 	 ...
 	 ddh(j) = round(peso_e(j)*(cq)*DHH(j)*sqrt(2.0*9.81*
	1 DHH(j))*DT/lato_cella,round_term)
...
!$OMP atomic
     ...
     1ddh(j)*Conc(ic,ir)   ! bifase
      ...
      !  aggiunto 8/10/2019
      U_stramaz(j) = round(ddh(j)*lato_cella/(DHH(j)*DT),round_term)
...
c  point 4.4.5.2    deposito
     ...
     1   ,dh_eros(j),U_stramaz(j))
         ...
         dh_neg=round(dh_neg+dh_eros(j),round_term)
...
!$OMP& schedule(monotonic: dynamic)
      ...
      h_solido(icol,irow) = round(h_solido(icol,irow) +
     1dh_entrata_solido_sorg(icol,irow) + 
     1dh_entrata_solido(icol,irow) + dh_solido(icol,irow) +
     1C_fondo(icol,irow)*dh_sed(icol,irow),round_term)   ! modifica 6/12/2013
     
	h(icol,irow) = round(h(icol,irow) + dh(icol,irow) + dh_sed(icol,irow) + 
	1dh_entrata_unif(icol,irow) + dh_entrata_Bel(icol,irow) + 
     1dh_entrata_sorg(icol,irow),round_term)        
...
* ******************************************************************************
       ...
       subroutine Hydrograph_volume(t22,iiijj,V_sub,Vs_sub)   ! 29 Novemnbre 2012
       ...
       if (t_fin(iiijj).gt.t22) then
            
          do j = 1, N_step_input(iiijj)-1
            
       if (t_1d(iiijj,j).le.t22.and.t_1d(iiijj,j+1).gt.t22) j1 = j  ! modificato 29/11
          ...
          if (j1.gt.1) then
           ...
           V_sub = V_sub + 0.5*(Q_input(iiijj,j) + 
     1Q_input(iiijj,j+1))*(t_1d(iiijj,j+1) - t_1d(iiijj,j))

           Vs_sub = Vs_sub + 0.5*(Q_input(iiijj,j)*Conc_input(iiijj,j) +
     1Q_input(iiijj,j+1)*Conc_input(iiijj,j+1))*(t_1d(iiijj,j+1) - 
     1t_1d(iiijj,j))
       ...
       Q2 = Q_input(iiijj,j1+1)-(Q_input(iiijj,j1+1)-Q_input(iiijj,j1))
     1*(t_1d(iiijj,j1+1)-t22)/(t_1d(iiijj,j1+1)-t_1d(iiijj,j1))
      
           V_sub = V_sub + 0.5*(Q2 + Q_input(iiijj,j1))*
     1(t22 - t_1d(iiijj,j1))
     
      CQ2 = Q_input(iiijj,j1+1)*Conc_input(iiijj,j1+1) - 
     1(Q_input(iiijj,j1+1)*Conc_input(iiijj,j1+1) - Q_input(iiijj,j1)*
     1Conc_input(iiijj,j1))*
     1(t_1d(iiijj,j1+1)-t22)/(t_1d(iiijj,j1+1)-t_1d(iiijj,j1))
     
      Vs_sub = Vs_sub + 0.5*(CQ2+Q_input(iiijj,j1)*Conc_input(iiijj,j1))
     1*(t22 - t_1d(iiijj,j1))
         ...
         do j = 1, N_step_input(iiijj)-1
           ...
           V_sub = V_sub + 0.5*(Q_input(iiijj,j) + Q_input(iiijj,j+1))*
     1(t_1d(iiijj,j+1) - t_1d(iiijj,j))
     
           Vs_sub = Vs_sub + 0.5*(Q_input(iiijj,j)*Conc_input(iiijj,j)+ 
     1Q_input(iiijj,j+1)*Conc_input(iiijj,j+1))*
     1(t_1d(iiijj,j+1) - t_1d(iiijj,j))
...
1051  externalValue=0.0/0.0
      ...
      real function round(value,cifre)
      implicit none
      real :: value
      integer cifre
      
      round = anint(value*10.**cifre)/(10.**cifre)
      !if(value*10.**cifre-anint(value*10.**cifre).ge.0.5)then
      !    round = round + 
      !endif
